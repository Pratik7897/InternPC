...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async (e) => {
    ...
    // If successful, page will redirect so no need to setIsOAuthLoading(false)
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-background relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-accent-blue/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-accent-teal/10 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      >
        ...
        <Link to="/" className="text-center block mb-8">
          <div className="w-16 h-16 mx-auto bg-white/10 rounded-xl flex items-center justify-center border border-white/20 shadow-[var(--glow)] mb-4">
        ...
        <div className="glass-card p-8">
          ...
          {/* Role Toggle */}
          <div className="flex p-1 bg-white/5 rounded-lg border border-white/10 mb-8 relative">
            <div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white/10 rounded-md transition-all duration-300 ${isStudent ? 'left-1' : 'left-[calc(50%+2px)]'} shadow-sm border border-white/10`}></div>
            <button 
              className={`flex-1 py-2 text-sm font-medium relative z-10 transition-colors ${isStudent ? 'text-white' : 'text-text-secondary hover:text-white'}`}
